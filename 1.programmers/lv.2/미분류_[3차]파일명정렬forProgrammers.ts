...
const 파일명정렬forProgrammers = (files: string[]): string[] => {
  const lodashLikeWords = (str: string): string[] => {
    ...
    const arrLastIndex = [...str].slice(arrIndex, arrIndex + number.lastIndexOf(false) + 1);
  ...
  arr.map((inArr) => inArr.sort((file1, file2) => parseInt(file1[1], 10) - parseInt(file2[1], 10)));
  const flatArr = arr.flat();
  return flatArr.map((fileKey) => files[map.get(fileKey)]);
};

const forPfiles1 = ['i mg12.png', 'i mg10.png', 'i mg02.png', 'i mg1.png', 'I MG01.GIF', 'i mg2.JPG'];
const forPfiles2 = ['F-5 Freedom Fighter', 'B-50 Superfortress', 'A-10 Thunderbolt II', 'F-14 Tomcat'];
